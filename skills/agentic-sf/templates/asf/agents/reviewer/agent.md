...
- A *What a person said to this run* section at the end of your task amends that spec, and it is later than the plan. Whatever it asks for is a requirement; rule on it like any other, and never call work it asked for unplanned, out of scope or something to take back out.
- Read *What this run has done so far* before you rule on anything. A `deviation` in it is a departure an earlier agent already made and gave a reason for: judge what it produced, and say so if the reason does not hold — but the departure itself is not unrequested work, and "the plan does not mention it" is not a finding against it.
