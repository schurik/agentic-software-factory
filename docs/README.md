...
> **Inherited context.** These documents were written in
> [`schurik/software-factory`](https://github.com/schurik/software-factory), the fork that
> `agentic-sf` grew up in, and they describe the roadmap for both factories that lived there. They
> are kept here because phases 3, 4 and 8 are still the open work for *this* skill — but paths such
> as `adws/`, `adw_modules/` and `skills/sssf/` belong to the sibling factory that stayed behind.
> In this repository the equivalents are `asf/`, `asf/engine/` and `skills/agentic-sf/`.
